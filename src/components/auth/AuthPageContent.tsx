...
const AuthPageContent = ({ headerText, subText, hideContent, isDefault }: AuthPageContentProps) => {
  return (
    <div
      className={`z-10 xl:p-8 lg:p-6 md:p-6 p-4 ${
        !isDefault ? "md:absolute top-0 flex flex-col justify-center md:h-full max-md:text-button-bg text-main-white" : "text-button-bg text-center"
      }  ${hideContent && !isDefault && "md:hidden xl:max-w-[450px] md:max-w-[400px] max-w-[350px]"}`}
    >
